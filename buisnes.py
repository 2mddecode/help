...
@bl.message(text="бизнес имя <name>")
async def buisnes_name(message: Message, name):
    ...
    if user['vip'] >= 2:
        if len(name) <= 10:
            ...
            return f"Установил имя бизнесу: {name}"
        else:
...
async def buisnes_put(message: Message):
    ...
    if info[0] == "OK":
        ...
        timeEnd = round((user['buisnes_time'] - time.time()) / 60)
        return f'За последний час ты уже снимал(а) деньги с бизнеса\n Осталось {timeEnd} минут(ы)'
    elif info[0] == "NOT":
    ...
    if a >= 1 and a <= 9:
        ...
        if user['buisnes_lvl'] == 0:
            if user['balance'] >= buy_lvl[index]:
                user['buisnes_time'] = time.time() + 3600
                user['buisnes_lvl'] = a
